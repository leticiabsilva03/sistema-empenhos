...
public class EmpenhoService {
    ...
    public EmpenhoEntity save(EmpenhoRecordDto empenhoRecordDto){
        ...
        empenhoEntity.setStatusEmpenho(EmpenhoEntity.Status.ABERTO);
    ...
    public void delete(Long id){
        EmpenhoEntity empenho = empenhoRepository.findById(id)
                .orElseThrow(()->new EmpenhoNotFoundException("Empenho não encontrado!"));
        if (empenho.getStatusEmpenho() == EmpenhoEntity.Status.LIQUIDADO){
            throw new EmpenhoStatusException("Não é possível excluir empenho liquidado. Estorne a liquidação antes.");
        }
        if (empenho.getStatusEmpenho() == EmpenhoEntity.Status.PAGO){
            throw new EmpenhoStatusException("Não é possível excluir empenho pago. Estorne o pagamento e a liquidação antes.");
        }
